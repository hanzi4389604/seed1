...
from swin_transformer1_swin_19c import SwinTransformer
...
#    model = ModelFactory.create(**config.model)
   ...
   # model.load_state_dict(torch.load('./swin_tiny_patch4_window7_224.pth')['model'],strict=False)
    model.head = nn.Linear(model.head.in_features, 51)
    print(model)
    ...
    train_loader, valid_loader , valid_loader2= DatasetFactory.create(**config.dataset)

    trainer = Trainer(model, optimizer, train_loader, valid_loader, scheduler, device, config.output_dir, valid_loader2)
